...

            {
                ...
                switch (option)
                {
                    ...
                    case SearchOptions.EMAILADDRESS:
                        if (filter == null)
                        {
                            filter = Builders<PhoneBook.Abstractions.Model.PhoneBook>.Filter.Eq("bookDetail.ownerEmail", parameter.Value.ToLower());
                        }
                        else
                        {

                            filter = Builders<PhoneBook.Abstractions.Model.PhoneBook>.Filter.Eq("bookDetail.ownerEmail", parameter.Value.ToLower()) & filter;
